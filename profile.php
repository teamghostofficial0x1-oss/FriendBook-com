...
// Dynamic routing query check for different profiles
...
if(!$user_info) { die("System Registry Database Fault: User not found."); }
...
<head>
    ...
    <title>Profile Hub - FriendBook</title>
...
<body class="bg-[#18191a] text-[#e4e6eb] min-h-screen font-sans">
    ...
    <nav class="sticky top-0 z-50 bg-[#242526] border-b border-[#393a3b] h-14 flex items-center justify-between px-4 shadow-md">
        ...
        <a href="feed.php" class="bg-[#3a3b3c] hover:bg-[#4e4f50] px-4 py-2 rounded-xl text-xs font-bold text-white"><i class="fas fa-home mr-1"></i> NewsFeed</a>
    </nav>
    ...
    <div class="max-w-2xl mx-auto mt-6 p-4">
        <div class="bg-[#242526] rounded-2xl border border-[#2f3031] p-6 text-center space-y-4 shadow-xl">
            ...
            <div class="relative w-32 h-32 mx-auto group">
                <img src="<?php echo $u_pic; ?>" class="w-full h-full object-cover rounded-full border-4 border-[#1877f2]">
                ...
                <form action="profile.php" method="POST" enctype="multipart/form-data" class="absolute inset-0 flex items-center justify-center bg-black/50 rounded-full opacity-0 group-hover:opacity-100 transition-all">
                    <label class="cursor-pointer text-white text-xs font-bold p-2">
            ...
            <div>
                <h2 class="text-2xl font-black text-white flex items-center justify-center gap-2">
                    ...
                    <?php if($user_info['username'] === 'adminRubel'): ?>
                        <span class="bg-red-600 text-white text-[10px] px-2 py-0.5 rounded font-black uppercase"><i class="fas fa-crown"></i> Admin</span>
                    <?php elseif($user_info['is_verified'] == 1): ?>
                        <span class="bg-[#1877f2] text-white text-xs w-5 h-5 flex items-center justify-center rounded-full" title="Global Verified Account"><i class="fas fa-check text-[9px]"></i></span>
                    <?php endif; ?>
                </h2>
