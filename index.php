...
// 🍪 অটো-লগইন চেক (৭ দিনের Remember Me কুকি থাকলে সরাসরি ফিডে যাবে)
...
// ইউজার যদি অলরেডি লগইন থাকে, তাকে ইন্ডেক্সে না রেখে ফিড পেজে রিডাইরেক্ট করা
if (isset($_SESSION['username'])) {
    header("Location: feed.php");
    exit;
}

// 🚪 ম্যানুয়াল লগআউট হ্যান্ডলার
...
$error = ''; $success = '';
$view = $_GET['view'] ?? 'login'; // login, register, forget, reset
...
// --- ২. লগইন প্রসেস (With 7 Days Persistence) ---
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reset_password'])) {
    ...
    if ($stmt->fetch()) {
        ...
        $success = "Password reset success! Log in with your new password.";
        $view = 'login';
    } else { $error = "Token is invalid or has already expired!"; }
}
?>
<!DOCTYPE html>
<html lang="en">
...
<body class="bg-[#18191a] text-[#e4e6eb] min-h-screen flex items-center justify-center p-4">
    ...
    <div class="bg-[#242526] p-6 rounded-xl border border-[#393a3b] w-full max-w-md shadow-2xl transition-all">
        ...
        <?php if($error): ?><div class="bg-red-500/10 text-red-400 p-3 rounded mb-4 text-xs border border-red-500/20 font-medium"><i class="fas fa-exclamation-triangle mr-2"></i><?php echo $error; ?></div><?php endif; ?>
        <?php if($success): ?><div class="bg-green-500/10 text-green-400 p-3 rounded mb-4 text-xs border border-green-500/20 font-medium"><i class="fas fa-check-circle mr-2"></i><?php echo $success; ?></div><?php endif; ?>
        ...
        <?php if($view === 'login'): ?>
            <form action="index.php?view=login" method="POST" class="space-y-4">
                ...
                <button type="submit" name="login" class="w-full bg-[#1877f2] py-2.5 rounded-lg font-bold text-sm hover:bg-[#1565c0] transition-colors shadow-md">Log In</button>
                <div class="text-center text-sm pt-2">
                    ...
                    <a href="index.php?view=register" class="bg-[#42b72a] px-5 py-2.5 rounded-lg text-sm font-bold text-white inline-block hover:bg-[#369622] transition-colors shadow-md">Create new account</a>
                </div>
            </form>

        <?php elseif($view === 'register'): ?>
            <form action="index.php?view=register" method="POST" class="space-y-4">
                <input type="text" name="username" placeholder="Choose Username (e.g. CyberNinja)" required class="w-full bg-[#3a3b3c] p-3 rounded outline-none text-sm">
        ...
        <?php elseif($view === 'forget'): ?>
            <form action="index.php?view=forget" method="POST" class="space-y-4">
                <p class="text-xs text-gray-400 leading-relaxed">Enter your registered email address below. We will simulate and push a secure reset link instantly.</p>
